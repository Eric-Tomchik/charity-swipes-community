...
The provider is registered in `convex/viktorSpaceAuthConfig.ts` from the deployment env (`VIKTOR_AUTH_CLIENT_ID`, `VIKTOR_AUTH_RESOURCE_ID`, `VIKTOR_AUTH_BASE_URL`); the OAuth config itself lives in `src/lib/viktor-spaces-access/authjs.ts`. Don't hand-roll OAuth code — Convex Auth handles redirects, PKCE, state, and the code exchange.

### Automatic sign-in intent param (`?viktor_sign_in=auto`)

Links to this app may carry `?viktor_sign_in=auto` (Viktor appends it to links it shares with workspace members). When a page load arrives with that param and no session exists, `src/components/ViktorAutoSignIn.tsx` starts the same "Sign in with Viktor" OAuth round trip automatically — invisible for workspace members with a live Viktor session. The attempt fires at most once per page load, never while a recent attempt is in flight, and the return leg's URL never carries the param, so a denial ends on the login page with the failure message (`?viktor_sign_in=failed`) instead of retrying. Visitors already signed in are taken straight to the app. The param is generated by Viktor — the app never appends it to its own URLs.
